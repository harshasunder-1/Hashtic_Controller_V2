...
__weak void TSK_MediumFrequencyTaskM1(void)
{
  ...
  case IDLE:
    if ( EAC_GetRestartState( &EncAlignCtrlM1 ) )
    {
      ...
      valFromSPI = ENC_getRawReadingFromSPI();
      ms16Val = ENC_makeRaw_mechS16(valFromSPI);
      ENC_SetMecAngle(&ENCODER_M1, ms16Val);
...
float BackEMFInVolts, temp1, temp2;
uint16_t temp3;
extern float k;
...
inline uint16_t FOC_CurrControllerM1(void)
{
  ...
  /**/
  if(CircleLimitationState == 0)
  {
    if(k<0.95f)
    {
      if(FOCVars[M1].Vqd.d < 0)
      {
        pPIDId[M1]->hUpperOutputLimit = -FOCVars[M1].Vqd.d;
        pPIDId[M1]->hLowerOutputLimit = FOCVars[M1].Vqd.d; 
      }
      else
      {
        pPIDId[M1]->hUpperOutputLimit = FOCVars[M1].Vqd.d;
      ...
      if(FOCVars[M1].Vqd.q < 0)
      {
        pPIDIq[M1]->hUpperOutputLimit = -FOCVars[M1].Vqd.q;
        pPIDIq[M1]->hLowerOutputLimit = FOCVars[M1].Vqd.q;
      }
      else
      {
        pPIDIq[M1]->hUpperOutputLimit = FOCVars[M1].Vqd.q;
      ...
      if(FOCVars[M1].Iqdref.q < 0)
      {
        PID_SetUpperOutputLimit(&PIDSpeedHandle_M1, -FOCVars[M1].Iqdref.q);
        PID_SetLowerOutputLimit(&PIDSpeedHandle_M1, FOCVars[M1].Iqdref.q);
      }
      else
      {
        PID_SetUpperOutputLimit(&PIDSpeedHandle_M1, FOCVars[M1].Iqdref.q);
  ...
  else
  {
    if(k>1.05f)
    {
      pPIDId[M1]->hUpperOutputLimit = INT16_MAX;
      pPIDIq[M1]->hUpperOutputLimit = INT16_MAX;
      
      pPIDId[M1]->hLowerOutputLimit = -INT16_MAX;
      pPIDIq[M1]->hLowerOutputLimit = -INT16_MAX;
  ...
  /******* Vs limiting functions *****************/
  BackEMFInVolts = (uint16_t)(((float)ss.currentAbsRpm) * 0.02f); 
  temp1 = (BackEMFInVolts/ss.DC_VOLTAGE );
  temp2 = temp1 + 0.25f;
  if(temp2>0.95f){ // 95% max voltage only
    temp2 = 0.95f;
  }
  if(temp2 < -0.95f){
    temp2 = -0.95f;
  }
  temp3 = temp2 * 32767.0f;
  
  pCLM[M1]->MaxModule = temp3; // This limit applied across the circle limitation.! // You can Hardcoding value - 7000 instead of temp3 to see it working
  /******* Vs limiting functions *****************/
  Vqd = Circle_Limitation_New(pCLM[M1], Vqd); // Changed the fns from Circle_Limitation to Circle_Limitation_New 
