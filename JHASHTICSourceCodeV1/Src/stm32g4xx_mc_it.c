...
uint8_t directionChange = 0,Enc_indexErrFlag=0,Enc_transitionErrFlag=0;
void SPD_TIM_M1_IRQHandler(void)
{
 ...
 /* Encoder Timer UPDATE IT is dynamicaly enabled/disabled, checking enable state is required */
  ...
  if (LL_TIM_IsActiveFlag_DIR(ENCODER_M1.TIMx)){
    encFlts.directionErrorCount++;
    if (encFlts.directionErrorCount > 3){
      encFlts.directionErrFlag=1;
      encFlts.EncoderErrFlag = 1;
    }
    LL_TIM_ClearFlag_DIR(ENCODER_M1.TIMx);
  }

  if (LL_TIM_IsActiveFlag_IERR(ENCODER_M1.TIMx)){
    encFlts.indexErrFlag=1;
    encFlts.EncoderErrFlag = 1;
    LL_TIM_ClearFlag_IERR(ENCODER_M1.TIMx);
    }
  
  if (LL_TIM_IsActiveFlag_TERR(ENCODER_M1.TIMx)){
    encFlts.transitionErrFlag=1;
    encFlts.EncoderErrFlag = 1;
    LL_TIM_ClearFlag_TERR(ENCODER_M1.TIMx);
  }
