...
// GLOBAL ABORT: If PCM drops the enable line, kill the sequence
  /*      if (!PCM_Start_Enable) {
            stopMotor();
            currentState = IDLE;
            break;
  ...
  switch (ssq_->currentState) {
    ...
    case IDLE_WAITING:
        if (ssq_->PCM_startCommand == 1) { 
            ...
            MC_ProgramTorqueRampMotor1(START_TORQUE_IQ*ss->direction,T_RAMP_DONE);
            MC_StartMotor1();
            ssq_->globalStartSeqTimer = 0;
    ...
    case START_SEQUENCE:
        ...
        if (ss->currentAbsRpm >= RPM_HANDOVER) {
          ...
          // Bumpless transfer with clamping
            ...
            int32_t integralTerm = kiDivisor * START_TORQUE_IQ * ss->direction;
            PID_SetIntegralTerm(&PIDSpeedHandle_M1,integralTerm);
            //SpeedPI.IntegralAccumulator = clamp(FOC.iqRef, SpeedPI.OutMin, SpeedPI.OutMax); 
            
            ss->targetRPM = RPM_STEADYSTATE;
            MC_ProgramSpeedRampMotor1(ss->targetRPM /6 * ss->direction, 1100 );
