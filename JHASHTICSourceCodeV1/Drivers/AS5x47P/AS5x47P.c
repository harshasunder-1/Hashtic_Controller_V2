...
uint8_t SPI_CheckReadForError(uint16_t data){
	return (data & 0x4000)>>14; // check if 15th bit is 1.if its 1, means error, else OK
}
...
uint16_t recievedFrame = 0,receivedData = 0,writtenData = 0;
...
uint8_t AS5047_SPI_Write(uint16_t addressFrame, uint16_t valueFrame) {
  ...
  receivedData = recievedFrame & 0x3FFF;
  writtenData = valueFrame & 0x3FFF;
...
uint16_t AS5047_SPI_Read(uint16_t command, uint8_t continuousRead) {
  ...
  HAL_GPIO_WritePin(SPI1_CS_GPIO_Port, SPI1_CS_Pin, GPIO_PIN_RESET);
  HAL_SPI_TransmitReceive(&hspi1, (uint8_t*)&txbuf, (uint8_t*)&rxbuf, 2,1000);
  ...
  if (continuousRead ==0){
	  //this is the NOP command frame for receiving data if you want to read the register once.
          ...
          txbuf[0] = NOP_FRAME >> 8;
	  txbuf[1] = NOP_FRAME & 0xFF;
	  HAL_SPI_TransmitReceive(&hspi1, (uint8_t*)&txbuf, (uint8_t*)&rxbuf, 2,0xFFFFFFFF);
	  while( hspi1.State == HAL_SPI_STATE_BUSY );
	  HAL_GPIO_WritePin(SPI1_CS_GPIO_Port, SPI1_CS_Pin, GPIO_PIN_SET);
  }
...
// changed spi_Read_function with error handler. gives -1 if error.
uint16_t AS5047_SPI_Read(uint16_t command, uint8_t continuousRead){
    ...
    /* SPI Transfer */
    status = HAL_SPI_TransmitReceive(&hspi1,txbuf,rxbuf,2,1000);
    for(int i = 0; i < 10000; i++){
      if (status == HAL_OK){break;}
    }
    HAL_GPIO_WritePin(SPI1_CS_GPIO_Port,SPI1_CS_Pin,GPIO_PIN_SET);

    for(int i = 0; i < 10000; i++){ //delay btw 2 readings
        __NOP();
    }
    ...
    finalDataVal = ((uint16_t)rxbuf[0] << 8) | rxbuf[1];
    return finalDataVal;
}
